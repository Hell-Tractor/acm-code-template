...
\begin{document}
  ...
  \section{最短路}
  \subsection{Dijkstra}
  \code{Hell Tractor}{优先队列优化Dijkstra}{MlogM}{N + M}{codes/dijkstra.cpp}
  \section{Tarjan}
  \subsection{SCC}
  \code{Hell Tractor}{Tarjan求强连通分量}{N + M}{N + M}{codes/tarjan_scc.cpp}
  \subsection{BCC}
  \code{Hell Tractor}{Tarjan求边双连通分量}{N + M}{N + M}{codes/tarjan_bcc.cpp}
  \section{网络流}
  \subsection{ISAP最大流}
  ...
  \section{模拟退火}
  \code{Hell Tractor}{使用时派生SA类，重写函数J, getNewState, checkState(可选重写)以及构造函数 \\  并需要设定type为所需求值类型}{Metaphysics}{1}{codes/SA.cpp}
  \section{IO}
